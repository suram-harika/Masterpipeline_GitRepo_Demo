param(
    [string]$SheduleserverExeMapping, [string]$ExeSharedLocation
)

Foreach ($row in (Get-Content -Path $SheduleserverExeMapping | Where {$_ -notmatch '^#.*'}|Select-String -Pattern (Get-ChildItem -Path $ExeSharedLocation| %{$_.Name}))) 
{ 
