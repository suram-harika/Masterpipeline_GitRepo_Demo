...
$UICONFIG_PATH=$Env_Config_Path.UICONFIG_PATH
$UICONFIG_BAT=$Env_Config_Path.UICONFIG_BAT
...
$filenames = Get-ChildItem -Path $pwd\$REPOSITORY\$UICONFIG_PATH -Recurse -Name -attributes !Directory | Where-Object {$_ -match "\.xml$"} 
$error_level=0
...
foreach ($filename in $filenames)
{
    ...
    if ($only_filename)
    {
        ...
        if ($found) 
        {
            Write-Host "Inside Found"
            $match=$found
            #$found = $found -replace "AWC/" , ""
            $found = $found -replace "/$only_filename" , ""

            Write-Host "Found value" $found "folder"
            if ($found -eq "$UICONFIG_PATH" )
            {
                $command = "$(pwd)/$REPOSITORY/$UICONFIG_PATH/$UICONFIG_BAT `"$only_filename`" $TC_ROOT"
                

                If($env:dryrun -eq "yes")
                {
                    Add-content $env:logdir/DryRunReport.txt "Importing AWC UI_Config: $command"
                }
                else
                {
                    Write-Host  "Execute cmd="  $command
                    $result = Invoke-Expression $command
                    if($result.contains("Failed to import AWC UI_Config"))
                    {
                        Write-Host $result
                        $error_level=1
                    }
            ...
            else
            {
                if (($siteName.Length -eq 2 )  -and ($siteName[1]))
                {
                    ...
                    {
                        $command = "$(pwd)/$REPOSITORY/$UICONFIG_PATH/$UICONFIG_BAT `"$only_filename[$only_filename.Length-1]`" $TC_ROOT"
                        ...
                        {
                            Write-Host  "Execute cmd Site Sepecific ="  $command
                            $result = Invoke-Expression $command
...
{
    Write-Host "Importing of AWC UI config failed"
    exit 1
}
