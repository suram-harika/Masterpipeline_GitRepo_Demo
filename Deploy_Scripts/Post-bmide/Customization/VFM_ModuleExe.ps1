param(
    [string]$VFM_ModuleExeMapping, [string]$ExeSharedLocation
)

Foreach ($row in (Get-Content -Path $VFM_ModuleExeMapping | Where {$_ -notmatch '^#.*'}|Select-String -Pattern (Get-ChildItem -Path $ExeSharedLocation| %{$_.Name}))) 
{ 
