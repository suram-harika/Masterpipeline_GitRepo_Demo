param(
    [string]$POOLExeMapping, [string]$ExeSharedLocation
)

Foreach ($row in (Get-Content -Path $POOLExeMapping | Where {$_ -notmatch '^#.*'}|Select-String -Pattern (Get-ChildItem -Path $ExeSharedLocation| %{$_.Name}))) 
{ 
