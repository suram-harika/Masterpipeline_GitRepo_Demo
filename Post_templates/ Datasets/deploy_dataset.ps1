...
$Datasets_path=$Env_Config_Path.Datasets_path
$Datasets_bat=$Env_Config_Path.Datasets_bat
...
$filenames = Get-ChildItem -Path $pwd\$REPOSITORY\$Datasets_path -Recurse -Name -attributes !Directory | Where-Object {$_ -match "\.xml$"} 
...
foreach ($filename in $filenames)
{
    ...
    if ($full_name)
    {
        $full_name = "$Datasets_path/$full_name"
        ...
        if ($found) 
        {
            ...
            if ($pathfolder -eq "$Datasets_path")
            {
                $command = "$(pwd)/$REPOSITORY/$Datasets_path/$Datasets_bat  $filename $TC_ROOT"
                If($env:dryrun -eq "yes")
                {
                    Add-content $env:logdir/DryRunReport.txt "Importing Datasets: $command"
                }
                else
                {
                    ...
                    if($result.contains("Failed to import Dataset"))
                    {
                        Write-Host $result
                        $error_level=1
                    }
...
{
    Write-Host "Dataset Import failed"
    exit 1
}
