...
$CLOSURERULE_PATH=$Env_Config_Path.CLOSURERULE_PATH
...
$filenames = Get-ChildItem -Path $pwd\$REPOSITORY\$CLOSURERULE_PATH -Recurse -Name -attributes !Directory | Where-Object {$_ -match "\.xml$"} 
...
foreach ($filename in $filenames)
{
    ...
    if ($full_name)
    {
        $full_name = "$CLOSURERULE_PATH/$full_name"
        ...
        if ($found) 
        {
            ...
            if ($pathfolder -eq "$CLOSURERULE_PATH")
            {
                $command = "$(pwd)/$REPOSITORY/$CLOSURERULE_PATH/$TRANSFERMODE_BAT  $filename $TC_ROOT"
                If($env:dryrun -eq "yes")
                {
                    Add-content $env:logdir/DryRunReport.txt "Importing closurerule: $command"
                }
                else
                {
                    Write-Host  "Execute cmd= "  $command
                    $result = Invoke-Expression $command
                    if($result.contains("Failed to import closurerule"))
                    {
                        Write-Host $result
                        $error_level=1
                    }
...
{
    Write-Host "Closurerule Import failed"
    exit 1
}
